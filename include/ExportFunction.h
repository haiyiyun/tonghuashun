...
typedef int(WINAPI *pfn_THS_EDBQuery)(IN const char *Indiactor, //指标
									  ...
									  IN const char *EndTime, char **ret);
...
typedef int(WINAPI *pfn_THS_Special_ShapePredict)(IN const char *THSCode,	//同花顺代码
												  IN const char *Param,		//函数需要的参数
												  IN const char *BeginTime, //请求的开始时间
												  IN const char *EndTime,	//请求的结束时间
												  OUT char **pRespData);	//返回结果

typedef int(WINAPI *pfn_THS_Special_StockLink)(IN const char *THSCode, //同花顺代码
											   IN const char *Param,   //函数需要的参数
											   OUT char **pRespData);  //返回结果

typedef int(WINAPI *pfn_THS_realTimeValuation)(IN const char *THSCode,	   //同花顺代码
											   IN const char *inputparam,  //函数需要的参数
											   IN const char *outputparam, //函数需要的参数
											   OUT char **pRespData);	   //返回结果
...
int THS_Special_ShapePredict(IN const char *THSCode,   //同花顺代码
							 IN const char *Param,	   //函数需要的参数
							 IN const char *BeginTime, //请求的开始时间
							 IN const char *EndTime,   //请求的结束时间
							 OUT char **pRespData);	   //返回结果

int THS_Special_StockLink(IN const char *THSCode, //同花顺代码
						  IN const char *Param,	  //函数需要的参数
						  OUT char **pRespData);  //返回结果

int THS_realTimeValuation(IN const char *THSCode,	  //同花顺代码
						  IN const char *inputparam,  //函数需要的参数
						  IN const char *outputparam, //函数需要的参数
						  OUT char **pRespData);	  //返回结果
